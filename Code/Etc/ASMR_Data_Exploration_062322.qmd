...
map_pop_control = function(sdf, col, brks = NULL) {
    sdf_selin = sdf %>% filter(grepl('^(11|23)', sgg_cd_c))
    sdf_buul = sdf %>% filter(grepl('^(21|26)', sgg_cd_c))
    ...
    mf_title(titletext, pos = "left", tab = TRUE, cex = 1.2, line = 1, inner=FALSE) 
    mf_inset_on(x = sdf_selin, pos = "topright", cex = .28)
    # display the target municipality
    mf_init(sdf_selin)
    mapsf::mf_choro(sdf, var = col, breaks = 'quantile', nbreaks = 5, leg_pos = NA, lwd = 0.1, add = TRUE)
...
for (i in colnames(fc0010_ncc)[-grep('(sgg_cd_c|geom)', colnames(fc0010_ncc))]) {
    map_pop_control(fc0010_ncc, i, brks_incmor)
}
```
...
#| layout-ncol: 3
for (i in colnames(fc0010_crr)[-grep('(sgg_cd_c|geom)', colnames(fc0010_crr))]) {
    map_pop_control(fc0010_crr, i)
}
```
...
#| layout-ncol: 3
for (i in colnames(fc0010_asr)[-grep('(sgg_cd_c|geom)', colnames(fc0010_asr))]) {
    map_pop_control(fc0010_asr, i)
}
```


```{r sgg sma, echo = F, fig.width=7.2, fig.height=6}
