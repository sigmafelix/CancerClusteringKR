...
sss_incidence_sf %>%
    ...
    filter(cancer == "Lung" & sex == "Male") %>%
    tm_shape(.) +
        tm_fill('prim_cluster', title = "Primary cluster", legend.show = FALSE, pal = c("#E15759"), showNA = FALSE, colorNA = "white", labels = c('Detected')) +
...
```{r envsensitivity-dataprep, fig.width = 12, fig.height = 7.5}
...
for (k in seq.int(1, nrow(sss_sens_incidence_sf_p))) {
    sss_sens_incidence_sf_p$geom[[k]] = sss_sens_incidence_sf_p$geom[[k]] + st_point(c(-1.35e5, 2e5), dim = 'XY')    
}
for (k in seq.int(1, nrow(sss_sens_mortality_sf_p))) {
    sss_sens_mortality_sf_p$geom[[k]] = sss_sens_mortality_sf_p$geom[[k]] + st_point(c(-1.35e5, 2e5), dim = 'XY')    
}
