...
```{r, echo = FALSE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, fig.width = 8, fig.height = 6, dev='ragg_png', dpi=300)
```
...
## Lung cancer (female)
```{r Lung (female), fig.width = 12, fig.height = 7.5, eval=T, include=T}
sss_incidence_sf_p = 
    sss_incidence_sf %>%
        mutate(prim_cluster = ifelse(prim_cluster == "FALSE", NA, prim_cluster)) %>%
...
```{r LISA plot, fig.width = 10.2, fig.height = 6}
lisa_incidence_sf = extr_lisa_sf(covar_origin_00_fc, covar_origin_05_fc, covar_origin_10_fc, "ragest_i_*.*male*")
lisa_mortality_sf = extr_lisa_sf(covar_origin_00_fc, covar_origin_05_fc, covar_origin_10_fc, "ragest_d_*.*male*")
...
```{r sensitivity plot Poisson, fig.width = 10.2, fig.height = 6}
...
csexlevels = c("Lung (Female)", "Lung (Male)", "Stomach (Female)", "Stomach (Male)")
