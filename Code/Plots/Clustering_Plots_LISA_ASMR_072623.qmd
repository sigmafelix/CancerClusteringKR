...
```{r, echo = FALSE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, fig.width = 8, fig.height = 6, dev='ragg_png', dpi=300, eval = FALSE)
```
...
- Change log
    ...
    - 12/17/2022
        - Restored female-lung cancer cases (for supplementary materials)
    - 05/21/2023
        - Changes to reflect comments
            - Remove legends in clustering results 
            - Additional sensitivity (environmental variable sets)
    - 05/25/2023
        - M3+NDVI, M3+exposure, and M3+emission
    - 06/18/2023
        - Color recoding (sex, cancer)
    - 07/23/2023
        - Color recoding (grayscale-compatible)
...
p_load(tidyverse, sf, stars, terra, starsExtra, readxl, here, tmap, patchwork, rmapshaper, spdep)
sf_use_s2(FALSE)
...
clear_input = function(filepath, alpha = 0.01, locmin = 2) {
    ...
    resl = split(res, res$analysis_title)
    return(resl)
}
...
p_v3nondvi_resl = clear_input(filepath = str_c(rdsdir, "SaTScan_Results/satscan_ASMR_nondvi_resid.rds"))
p_v3noapsum_resl = clear_input(filepath = str_c(rdsdir, "SaTScan_Results/satscan_ASMR_noapsum_resid.rds"))
p_v3noappred_resl = clear_input(filepath = str_c(rdsdir, "SaTScan_Results/satscan_ASMR_noappred_resid.rds"))

p_v3ndvi_resl = clear_input(filepath = str_c(rdsdir, "SaTScan_Results/satscan_ASMR_M3ndvi_resid.rds"))
p_v3apsum_resl = clear_input(filepath = str_c(rdsdir, "SaTScan_Results/satscan_ASMR_M3apsum_resid.rds"))
p_v3appred_resl = clear_input(filepath = str_c(rdsdir, "SaTScan_Results/satscan_ASMR_M3appred_resid.rds"))


```

```{r colorsetting, eval=T}
# red and orange for female and male lung cancer, and blue and light blue for female and male stomach cancer
# OLD set: #e31a1c, #ff7f00, #1f78b4, #a6cee3
# NEW set: #66C5CC,#F6CF71,#F89C74,#DCB0F2
color_list = list(
    lung.female = "#66C5CC", 
    lung.male = "#F6CF71", 
    stomach.female = "#F89C74", 
    stomach.male = "#DCB0F2"
)
...
## Cleaning functions
```{r Lung cancer cluster plots (incidence), fig.width=12, fig.height = 5.6, eval=T}
...
sss_incidence_sf = right_join(base_sgg, ref_check) %>%
    ...
    mutate(nflag = str_replace(nflag, "N=", "|C|="))
...
for (k in seq.int(1, nrow(sss_incidence_sf_p))) {
    sss_incidence_sf_p$geom[[k]] = sss_incidence_sf_p$geom[[k]] + st_point(c(-1.50e5, 2e5), dim = 'XY')    
}
for (k in seq.int(1, nrow(sss_mortality_sf_p))) {
    sss_mortality_sf_p$geom[[k]] = sss_mortality_sf_p$geom[[k]] + st_point(c(-1.50e5, 2e5), dim = 'XY')    
}
...
sss_incidence_sf %>%
    ...
    filter(cancer == "Lung" & sex == "Male") %>%
    tm_shape(.) +
        tm_fill('prim_cluster', title = "Primary cluster", legend.show = FALSE, 
                pal = color_list$lung.male,
                showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        ...
        tm_layout(panel.label.bg.color = 'transparent',
                  title = NULL,#"Stomach incidence rate\n(female)",
                  frame = FALSE,
                  legend.title.size = 1,
                  legend.text.size = 1,
...
## Lung cancer (female)
```{r Lung (female), fig.width = 12, fig.height = 7.5, include=T}
sss_incidence_sf_p = 
    sss_incidence_sf %>%
        mutate(prim_cluster = ifelse(prim_cluster == "FALSE", NA, prim_cluster)) %>%
...
```{r LISA plot, fig.width = 10.2, fig.height = 7}
lisa_incidence_sf = extr_lisa_sf(covar_origin_00_fc, covar_origin_05_fc, covar_origin_10_fc, "ragest_i_*.*male*")
lisa_mortality_sf = extr_lisa_sf(covar_origin_00_fc, covar_origin_05_fc, covar_origin_10_fc, "ragest_d_*.*male*")
# load(str_c(rdsdir, "Output/LISA_integrated_sf_091522.RData"))
safepal = c("#88CCEE", "#CC6677", "#DDCC77", "#117733", "#332288", "#AA4499", "#44AA99", "#999933", "#882255", "#661100", "#6699CC", "#888888", "#032903", "#d9073f", "#f5c8db")
# blues: 1, 11
# reds : 2, 9

#colpal = c("#4E79A7", "deepskyblue1", "pink", "#E15759")
colpal = safepal[c(13, 1, 15, 14)]
colpal = rev(colpal)
namepal = c("High-High", "High-Low", "Low-High", "Low-Low")

lisa_incidence_sf = lisa_incidence_sf %>%
    filter(!sgg_cd_c %in% c(23320, 37430, 39010, 39020))
lisa_mortality_sf = lisa_mortality_sf %>%
    filter(!sgg_cd_c %in% c(23320, 37430, 39010, 39020))
levels_cancersex = c("Lung (Male)", "Lung (Female)", "Stomach (Male)", "Stomach (Female)")
...
lisa_incidence_sf %>%
    ...
    tm_shape(.) +
        ...
        tm_layout(panel.label.bg.color = 'transparent',
                  ...
                  panel.label.size = 1.4,
                  fontfamily = "Pretendard",
                  outer.margins = c(0.005, 0.005, 0.005, -0.12))
...
```{r sensitivity plot Poisson, fig.width = 10.2, fig.height = 7, eval=T}
...
csexlevels = c("Lung (Male)", "Lung (Female)", "Stomach (Male)", "Stomach (Female)")
...
sssp_incidence_sf_va_p = 
    sssp_incidence_sf_va %>%
        ...
        mutate(cancersex = factor(cancersex, levels = csexlevels)) %>%
...
sssp_incidence_sf_va = sssp_incidence_sf_va %>%
    mutate(cancersex = ifelse(sex == "Male", str_c(cancer, " (Male)"), str_c(cancer, " (Female)"))) %>%
    mutate(csex2 = ifelse(prim_cluster == "FALSE", NA, cancersex))
sssp_mortality_sf_va = sssp_mortality_sf_va %>%
    mutate(cancersex = ifelse(sex == "Male", str_c(cancer, " (Male)"), str_c(cancer, " (Female)"))) %>%
    mutate(csex2 = ifelse(prim_cluster == "FALSE", NA, cancersex))


for (k in seq.int(1, nrow(sssp_incidence_sf_va_p))) {
    sssp_incidence_sf_va_p$geom[[k]] = sssp_incidence_sf_va_p$geom[[k]] + st_point(c(-1.50e5, 2e5), dim = 'XY')    
}
for (k in seq.int(1, nrow(sssp_mortality_sf_va_p))) {
    sssp_mortality_sf_va_p$geom[[k]] = sssp_mortality_sf_va_p$geom[[k]] + st_point(c(-1.50e5, 2e5), dim = 'XY')    
}



sssp_incidence_sf_va %>%
    # mutate(cancersex = ifelse(is.na(prim_cluster), NA, cancersex)) %>%
    mutate(cancersex = factor(cancersex, levels = csexlevels)) %>%
    # # filter((cancer != "Lung" | sex != "Female")) %>%
    tm_shape(.) +
        tm_fill('csex2', title = NA, legend.show = FALSE, pal = colorc, showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        # tm_fill('cancersex', title = NA, legend.show = FALSE, pal = colorc, showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "cancersex"), nrow = 3, ncol = 4) +
        ...
        tm_facets(by = c("year", "cancersex"), nrow = 3, ncol = 4) +
        tm_layout(panel.label.bg.color = 'transparent',
                  title = NA,#"Stomach incidence rate\n(female)",
                  frame = FALSE,
                  legend.title.size = 1,
                  legend.text.size = 1,
                  panel.label.size = 1.4,
...
sssp_mortality_sf_va %>%
    ...
    tm_shape(.) +
        tm_fill('csex2', title = NA, legend.show = FALSE, pal = colorc, showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        # tm_fill('prim_cluster', title = NA, legend.show = FALSE, pal = c("#E15759"), showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "cancersex"), nrow = 3, ncol = 4) +
...
# Full vs three environmental sets
```{r envsensitivity-dataprep, fig.width = 12, fig.height = 7.5, eval=F}

sss_incidence_sf_v3nondvi = extr_sss_sf(covar_origin_00_fc, p_v3nondvi_resl, "_i_") %>%
    mutate(vset = 'M4a: M4-NDVI')
sss_mortality_sf_v3nondvi = extr_sss_sf(covar_origin_00_fc, p_v3nondvi_resl, "_d_") %>%
    mutate(vset = 'M4a: M4-NDVI')
sss_incidence_sf_v3noappred = extr_sss_sf(covar_origin_00_fc, p_v3noappred_resl, "_i_") %>%
    mutate(vset = 'M4b: M4-Exposure')
sss_mortality_sf_v3noappred = extr_sss_sf(covar_origin_00_fc, p_v3noappred_resl, "_d_") %>%
    mutate(vset = 'M4b: M4-Exposure')
sss_incidence_sf_v3noapsum = extr_sss_sf(covar_origin_00_fc, p_v3noapsum_resl, "_i_") %>%
    mutate(vset = 'M4c: M4-Emission')
sss_mortality_sf_v3noapsum = extr_sss_sf(covar_origin_00_fc, p_v3noapsum_resl, "_d_") %>%
    mutate(vset = 'M4c: M4-Emission')

sss_sens_incidence_df = 
    rbind(sss_incidence_sf_v3,
          sss_incidence_sf_v3nondvi,
          sss_incidence_sf_v3noappred,
          sss_incidence_sf_v3noapsum) %>%
    mutate(sex = ifelse(grepl('_female_', analysis_title), 'Female', 'Male')) %>%
    st_drop_geometry
sss_sens_mortality_df = 
    rbind(sss_mortality_sf_v3,
          sss_mortality_sf_v3nondvi,
          sss_mortality_sf_v3noappred,
          sss_mortality_sf_v3noapsum) %>%
    mutate(sex = ifelse(grepl('_female_', analysis_title), 'Female', 'Male')) %>%
    st_drop_geometry

ref_sens_check = 
    expand.grid(sgg_cd_c = unique(sss_sens_incidence_df$sgg_cd_c),
                vset = c("M4: M3+Environment", 'M4a: M4-NDVI', 'M4b: M4-Exposure', 'M4c: M4-Emission'),
                year = c('1999-2003', '2004-2008', '2009-2013'),
                cancer = c('Stomach', 'Lung'),
                sex = c('Male', 'Female'))
base_sgg = covar_origin_00_fc %>%
...
sss_sens_incidence_sf = right_join(base_sgg, ref_sens_check) %>%
    left_join(sss_sens_incidence_df) %>%
    mutate(nflag = str_replace(nflag, "N=", "|C|="))
sss_sens_mortality_sf = right_join(base_sgg, ref_sens_check) %>%
    left_join(sss_sens_mortality_df) %>%
    mutate(nflag = str_replace(nflag, "N=", "|C|="))
...
## Lung cancer (male)
```{r sensitivity lung(male), eval=F}

sss_sens_incidence_sf_p = 
    sss_sens_incidence_sf %>%
        mutate(prim_cluster = ifelse(prim_cluster == "FALSE", NA, prim_cluster)) %>%
        filter(cancer == "Lung" & sex == "Male") %>%
        group_by(cancer, sex, year, vset) %>%
        summarize(nflag = unique(nflag)) %>%
...
sss_sens_mortality_sf_p = 
    sss_sens_mortality_sf %>%
        mutate(prim_cluster = ifelse(prim_cluster == "FALSE", NA, prim_cluster)) %>%
        filter(cancer == "Lung" & sex == "Male") %>%
        group_by(cancer, sex, year, vset) %>%
        summarize(nflag = unique(nflag)) %>%
...
for (k in seq.int(1, nrow(sss_sens_incidence_sf_p))) {
    sss_sens_incidence_sf_p$geom[[k]] = sss_sens_incidence_sf_p$geom[[k]] + st_point(c(-1.50e5, 2e5), dim = 'XY')    
}
for (k in seq.int(1, nrow(sss_sens_mortality_sf_p))) {
    sss_sens_mortality_sf_p$geom[[k]] = sss_sens_mortality_sf_p$geom[[k]] + st_point(c(-1.50e5, 2e5), dim = 'XY')    
}
...
sss_sens_incidence_sf %>%
    ...
    tm_shape(.) +
        # tm_fill('prim_cluster', title = NA, legend.show = FALSE, pal = c("#E15759"), showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        tm_fill('prim_cluster', title = NA, legend.show = FALSE, pal = color_list$lung.male, showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "vset"), nrow = 3, ncol = 4) +
        tm_shape(sss_sens_incidence_sf_p) +
        tm_text("nflag", size = 1.5, remove.overlap = TRUE) +
        tm_facets(by = c("year", "vset"), nrow = 3, ncol = 4) +
        tm_layout(panel.label.bg.color = 'transparent',
                  title = NA,#"Stomach incidence rate\n(female)",
...
sss_sens_mortality_sf %>%
    ...
    tm_shape(.) +
        ...
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "vset"), nrow = 3, ncol = 4) +
        tm_shape(sss_sens_mortality_sf_p) +
        tm_text("nflag", size = 1.5, remove.overlap = TRUE) +
        tm_facets(by = c("year", "vset"), nrow = 3, ncol = 4) +
        tm_layout(panel.label.bg.color = 'transparent',
                  title = NULL,#"Stomach mortality rate\n(female)",
                  frame = FALSE,
                  legend.title.size = 1,
                  legend.text.size = 1,
                  panel.label.size = 1.4,
...
## Lung cancer (female)
```{r sensitivity lung(female), eval=F}
...
sss_sens_incidence_sf %>%
    ...
    filter(cancer == "Lung" & sex == "Female") %>%
    tm_shape(.) +
        tm_fill('prim_cluster', title = NA, legend.show = FALSE, pal = color_list$lung.female, showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "vset"), nrow = 3, ncol = 4) +
        tm_shape(sss_sens_incidence_sf_p) +
        tm_text("nflag", size = 1.5, remove.overlap = TRUE) +
...
sss_sens_mortality_sf %>%
    ...
    tm_shape(.) +
        ...
        tm_facets(by = c("year", "vset"), nrow = 3, ncol = 4) +
        tm_layout(panel.label.bg.color = 'transparent',
                  title = NA,#"Stomach mortality rate\n(female)",
                  frame = FALSE,
                  legend.title.size = 1,
                  legend.text.size = 1,
                  panel.label.size = 1.4,
...
## Stomach cancer (male)
```{r sensitivity stomach(male), eval=F}
...
sss_sens_incidence_sf %>%
    ...
    filter(cancer == "Stomach" & sex == "Male") %>%
    tm_shape(.) +
        tm_fill('prim_cluster', title = NA, legend.show = FALSE, pal = color_list$stomach.male, showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "vset"), nrow = 3, ncol = 4) +
        tm_shape(sss_sens_incidence_sf_p) +
        tm_text("nflag", size = 1.5, remove.overlap = TRUE) +
...
```{r sensitivity stomach(female), eval=F}
...
sss_sens_incidence_sf %>%
    ...
    filter(cancer == "Stomach" & sex == "Female") %>%
    tm_shape(.) +
        tm_fill('prim_cluster', title = NA, legend.show = FALSE, pal = color_list$stomach.female, showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "vset"), nrow = 3, ncol = 4) +
        tm_shape(sss_sens_incidence_sf_p) +
        tm_text("nflag", size = 1.5, remove.overlap = TRUE) +
...
# M3 extension: adding one set at a time
```{r envsensitivity-dataprep2, fig.width = 12, fig.height = 7.5}

sss_incidence_sf_v3ndvi = extr_sss_sf(covar_origin_00_fc, p_v3ndvi_resl, "_i_") %>%
    mutate(vset = 'M3a: M3+NDVI')
sss_mortality_sf_v3ndvi = extr_sss_sf(covar_origin_00_fc, p_v3ndvi_resl, "_d_") %>%
    mutate(vset = 'M3a: M3+NDVI')
sss_incidence_sf_v3appred = extr_sss_sf(covar_origin_00_fc, p_v3appred_resl, "_i_") %>%
    mutate(vset = 'M3b: M3+Exposure')
sss_mortality_sf_v3appred = extr_sss_sf(covar_origin_00_fc, p_v3appred_resl, "_d_") %>%
    mutate(vset = 'M3b: M3+Exposure')
sss_incidence_sf_v3apsum = extr_sss_sf(covar_origin_00_fc, p_v3apsum_resl, "_i_") %>%
    mutate(vset = 'M3c: M3+Emission')
sss_mortality_sf_v3apsum = extr_sss_sf(covar_origin_00_fc, p_v3apsum_resl, "_d_") %>%
    mutate(vset = 'M3c: M3+Emission')

sss_sens_incidence_df = 
    rbind(sss_incidence_sf_v2,
          sss_incidence_sf_v3ndvi,
          sss_incidence_sf_v3appred,
          sss_incidence_sf_v3apsum) %>%
    mutate(sex = ifelse(grepl('_female_', analysis_title), 'Female', 'Male')) %>%
    st_drop_geometry
sss_sens_mortality_df = 
    rbind(sss_mortality_sf_v2,
          sss_mortality_sf_v3ndvi,
          sss_mortality_sf_v3appred,
          sss_mortality_sf_v3apsum) %>%
    mutate(sex = ifelse(grepl('_female_', analysis_title), 'Female', 'Male')) %>%
    st_drop_geometry

ref_sens_check = 
    expand.grid(sgg_cd_c = unique(sss_sens_incidence_df$sgg_cd_c),
                vset = c("M3: M2+Behavior", 'M3a: M3+NDVI', 'M3b: M3+Exposure', 'M3c: M3+Emission'),
                year = c('1999-2003', '2004-2008', '2009-2013'),
                cancer = c('Stomach', 'Lung'),
                sex = c('Male', 'Female'))
base_sgg = covar_origin_00_fc %>%
