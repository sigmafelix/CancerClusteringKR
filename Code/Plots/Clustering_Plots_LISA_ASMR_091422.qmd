...
## Lung cancer (female)
```{r Lung (female), fig.width = 14, fig.height = 9}
...
sss_incidence_sf_v4 %>%
    ...
    mutate(cancersex = factor(cancersex, 
        levels = c("Lung (Male)", "Lung (Female)", "Stomach (Male)", "Stomach (Female)"))) %>%
    # filter((cancer != "Lung" | sex != "Female")) %>%
    tm_shape(.) +
        tm_fill('prim_cluster', title = "Primary cluster", pal = c("#E15759"), showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        tm_borders('dark grey', lwd = 0.4) +
...
sss_mortality_sf_v4 %>%
    ...
    mutate(cancersex = ifelse(sex == "Male", str_c(cancer, " (Male)"), str_c(cancer, " (Female)"))) %>%
    mutate(cancersex = factor(cancersex, 
        levels = c("Lung (Male)", "Lung (Female)", "Stomach (Male)", "Stomach (Female)"))) %>%    # filter((cancer != "Lung" | sex != "Female")) %>%
    tm_shape(.) +
        tm_fill('prim_cluster', title = "Primary cluster", pal = c("#E15759"), showNA = FALSE, colorNA = "white", labels = c('Detected')) +
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "cancersex"), nrow = 2, ncol = 4) +
