...
ggcorr_multi(fcs, yvec_mlm, xvec_m1, xvec_m2, "ASMR\n(Lung, Male)", xvec_lab1, xvec_lab2, plabs,
             rdsdir, "Corrplot_Lung_Male_mor_112022.png")
ggcorr_multi(fcs, yvec_msm, xvec_m1, xvec_m2, "ASMR\n(Stomach, Male)", xvec_lab1, xvec_lab2, plabs,
             rdsdir, "Corrplot_Stomach_Male_mor_112022.png")
ggcorr_multi(fcs, yvec_msf, xvec_m1, xvec_m2, "ASMR\n(Stomach, Female)", xvec_lab1, xvec_lab2, plabs,
...
png(filename = str_c(rdsdir, "Manuscript/Total_corrplot_112122.png"), 
        width = 12, height = 12, units = 'in', pointsize = 10, res = 508,
        type = "cairo-png")
par(mfcol = c(2, 2))
corr_single(fcs[[1]], yvec_p1, xvec_m1, ylab_all, xvec_lab1, 'Period 1')
corr_single(fcs[[2]], yvec_p2, xvec_m2, ylab_all, xvec_lab2, 'Period 2')
corr_single(fcs[[3]], yvec_p3, xvec_m2, ylab_all, xvec_lab2, 'Period 3')
dev.off()
...
ref_check = 
    expand.grid(sgg_cd_c = unique(sss_incidence_df$sgg_cd_c),
                ...
                sex = c('Male', 'Female')) %>%
    mutate(year != "1999-2003" & vset != c("M3: M2+Behavior"))
...
## Sensitivity (LISA)
```{r LISA plot, fig.width = 9, fig.height = 6}
lisa_incidence_sf = extr_lisa_sf(covar_origin_00_fc, covar_origin_05_fc, covar_origin_10_fc, "ragest_i_*.*male*")
lisa_mortality_sf = extr_lisa_sf(covar_origin_00_fc, covar_origin_05_fc, covar_origin_10_fc, "ragest_d_*.*male*")
...
```{r sensitivity plot Poisson, fig.width = 9, fig.height = 6, eval=T}
...
## Sensitivity (M5)
```{r Sensitivity (m5), fig.width = 9, fig.height = 6, include=FALSE, eval = FALSE}
...
```{r Sensitivity (education only), fig.width = 9, fig.height = 6, eval=F}
...
sss_incidence_sf_ve_p = 
    sss_incidence_sf_ve %>%
        mutate(prim_cluster = ifelse(prim_cluster == "FALSE", NA, prim_cluster)) %>%
        filter(sex != "Female" | cancer != "Lung") %>%
        group_by(cancer, sex, year, cancersex, vset) %>%
        summarize(nflag = unique(nflag)) %>%
        ungroup %>%
        st_centroid %>%
        filter(!is.na(cancersex)) %>%
    mutate(cancersex = factor(cancersex, 
        levels = c("Lung (Male)", "Stomach (Male)", "Stomach (Female)"))) %>%
        mutate(cancersex = droplevels(cancersex))

sss_mortality_sf_ve_p = 
    sss_mortality_sf_ve %>%
        mutate(prim_cluster = ifelse(prim_cluster == "FALSE", NA, prim_cluster)) %>%
        filter(sex != "Female" | cancer != "Lung") %>%
        group_by(cancer, sex, year, cancersex, vset) %>%
...
for (k in seq.int(1, nrow(sss_incidence_sf_ve_p))) {
    sss_incidence_sf_ve_p$geom[[k]] = sss_incidence_sf_ve_p$geom[[k]] + st_point(c(-1.35e5, 2e5), dim = 'XY')    
}
for (k in seq.int(1, nrow(sss_mortality_sf_ve_p))) {
    sss_mortality_sf_ve_p$geom[[k]] = sss_mortality_sf_ve_p$geom[[k]] + st_point(c(-1.35e5, 2e5), dim = 'XY')    
}
...
sss_incidence_sf_ve %>%
    ...
    tm_shape(.) +
        ...
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "cancersex"), nrow = 3, ncol = 3) +
        tm_shape(sss_incidence_sf_ve_p) +
        tm_text("nflag", size = 1.5, remove.overlap = TRUE) +
        tm_facets(by = c("year", "cancersex"), nrow = 3, ncol = 3) +
        tm_layout(panel.label.bg.color = 'transparent',
                  title = "Incidence rate\n(Base+Education)",
...
sss_mortality_sf_ve %>%
    ...
    tm_shape(.) +
        ...
        tm_borders('dark grey', lwd = 0.4) +
        tm_facets(by = c("year", "cancersex"), nrow = 3, ncol = 3) +
        tm_shape(sss_mortality_sf_ve_p) +
        tm_text("nflag", size = 1.5, remove.overlap = TRUE) +
        tm_facets(by = c("year", "cancersex"), nrow = 3, ncol = 3) +
        tm_layout(panel.label.bg.color = 'transparent',
                  title = "Mortality rate\n(Base+Education)",
