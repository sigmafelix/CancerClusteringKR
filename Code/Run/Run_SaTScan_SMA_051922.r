...
covar_origin_10_fc = covar_origin_10_fc %>% bind_cols(as.data.frame(st_coordinates(st_centroid(., of_largest_polygon = TRUE))))

covar_origin_00_fcd = covar_origin_00_fc %>% 
    bind_cols(as.data.frame(st_coordinates(st_centroid(., of_largest_polygon = TRUE)))) %>%
    st_drop_geometry %>%
    dplyr::filter(grepl('^(11|23|31)', sgg_cd_c) & sgg_cd_c != 23320)
covar_origin_05_fcd = covar_origin_05_fc %>% 
    bind_cols(as.data.frame(st_coordinates(st_centroid(., of_largest_polygon = TRUE)))) %>%
    st_drop_geometry %>%
    dplyr::filter(grepl('^(11|23|31)', sgg_cd_c) & sgg_cd_c != 23320)
covar_origin_10_fcd = covar_origin_10_fc %>% 
    bind_cols(as.data.frame(st_coordinates(st_centroid(., of_largest_polygon = TRUE)))) %>%
    st_drop_geometry %>%
    dplyr::filter(grepl('^(11|23|31)', sgg_cd_c) & sgg_cd_c != 23320)
...
p1_all = mapply(function(x, y) {
    generate_satscan_prm(data = dat1,
                    ...
                    filename.input = "satscan_base_SMA_data_period1.csv",
...
p2_all = mapply(function(x, y) {
    generate_satscan_prm(data = dat2,
                    title.analysis = y,
                    dir.base = dbase, dir.target = "/home/felix/workcache/", name.idcol = geoid,
                    filename.input = "satscan_base_SMA_data_period2.csv",
                    filename.output = str_c(y, ".txt"),
                    col.var = str_c('ragest', x),
                    col.case = str_c('n', x),
                    prm.path = "/home/felix/Documents/test.prm"
...
p3_all = mapply(function(x, y) {
    generate_satscan_prm(data = dat3,
                    title.analysis = y,
                    dir.base = dbase, dir.target = "/home/felix/workcache/", name.idcol = geoid,
                    filename.input = "satscan_base_SMA_data_period3.csv",
                    filename.output = str_c(y, ".txt"),
                    col.var = str_c('ragest', x),
                    col.case = str_c('n', x),
...
p13_all_df = bind_rows(p1_all_df, p2_all_df) %>%
    bind_rows(p3_all_df)
write_rds(p13_all_df, "/mnt/c/Users/sigma/OneDrive/NCC_Project/CancerClustering/satscan_ASMR_SMA_uncontrolled_resid.rds")
...
write_rds(p13_all_v1_df, "/mnt/c/Users/sigma/OneDrive/NCC_Project/CancerClustering/satscan_ASMR_SMA_vset1_resid.rds")
...
write_rds(p13_all_v2_df, "/mnt/c/Users/sigma/OneDrive/NCC_Project/CancerClustering/satscan_ASMR_SMA_vset2_resid.rds")
...
write_rds(p13_all_v3_df, "/mnt/c/Users/sigma/OneDrive/NCC_Project/CancerClustering/satscan_ASMR_SMA_vset3_resid.rds")
...
write_rds(p3_all_v4_df, "/mnt/c/Users/sigma/OneDrive/NCC_Project/CancerClustering/satscan_ASMR_SMA_vset4_resid.rds")
